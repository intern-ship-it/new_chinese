...
class SalesController extends Controller
{
    ...
    {
        // Validate request - only accept minimal data from frontend
        $validator = Validator::make($request->all(), [
            ...
            // Items validation - only accept id, deity_id, quantity, and vehicles
            ...
            'devotee.name_primary' => 'nullable|string|max:255',
            'devotee.name_secondary' => 'nullable|string|max:255',
            ...
            // Payment validation - only payment mode
        ...
        try {
            ...
            // Fetch items from database and calculate subtotal
            $items = $request->input('items');
            $itemsWithDetails = [];
            $subtotal = 0;

            foreach ($items as $itemData) {
                // Fetch sale item from database
                $saleItem = SaleItem::find($itemData['id']);

                if (!$saleItem) {
                    return response()->json([
                        'success' => false,
                        'message' => "Sale item with ID {$itemData['id']} not found"
                    ], 422);
                }

                // Use database price (effective_price or price)
                $unitPrice = $saleItem->effective_price ?? $saleItem->price;
                $quantity = $itemData['quantity'];
                $itemTotal = $unitPrice * $quantity;

                // Store item with details from database
                $itemsWithDetails[] = [
                    'id' => $saleItem->id,
                    'deity_id' => $itemData['deity_id'] ?? null,
                    'name_primary' => $saleItem->name_primary,
                    'name_secondary' => $saleItem->name_secondary,
                    'short_code' => $saleItem->short_code,
                    'sale_type' => $saleItem->sale_type,
                    'price' => $unitPrice,
                    'quantity' => $quantity,
                    'total' => $itemTotal,
                    'vehicles' => $itemData['vehicles'] ?? null
                ];

                $subtotal += $itemTotal;
            }

            // Calculate amounts in backend
            $discountAmount = $request->input('discount_amount', 0);
            $depositAmount = $request->input('deposit_amount', 0);
            $totalAmount = $subtotal - $discountAmount;
            $paidAmount = $totalAmount; // Full payment for now
            ...
            // Create booking record with calculated amounts
            ...
            // Create booking items using database details
            ...
            // Create payment record with calculated paid amount
    ...
    {
        $metaData = [
            'devotee_name_primary' => $devotee['name_primary'] ?? null,
            'devotee_name_secondary' => $devotee['name_secondary'] ?? null,
