...
(function ($, window) {
  ...
  window.TempleRouter = {
    ...
    getCurrentPage: function () {
      ...
      // Handle member application routes
      if (pathParts[0] === "members" && pathParts[1] === "application") {
        if (pathParts.length === 2) {
          // members/application -> index
          return "members/application";
        } else if (pathParts.length === 3) {
          if (pathParts[2] === "form") {
            // members/application/form -> form
            return "members/application-form";
          } else if (pathParts[2] === "verify") {
            // members/application/verify -> verify (shouldn't happen without ID)
            return "members/application-verify";
          } else if (pathParts[2] === "view") {
            // members/application/view -> view (shouldn't happen without ID)
            return "members/application-view";
          } else {
            // members/application/{id} -> verify (default action)
            return "members/application-verify";
          }
        } else if (pathParts.length === 4) {
          if (pathParts[2] === "form") {
            // members/application/form/{id} -> form (edit mode)
            return "members/application-form";
          } else if (pathParts[2] === "verify") {
            // members/application/verify/{id} -> verify
            return "members/application-verify";
          } else if (pathParts[2] === "view") {
            // members/application/view/{id} -> view
            return "members/application-view";
          }
        }
      }
