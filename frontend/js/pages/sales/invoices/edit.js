// js/pages/sales/invoices/edit.js
// Edit Sales Invoice - with Packages and Addons sections (like Sales Order)
// FIXED: Tax calculation now computes tax_amount from tax_rate

(function ($, window) {
    'use strict';

    window.SalesInvoicesEditPage = {
        customers: [], // Devotees
        taxRates: [],
        packages: [],
        ...
        loadInvoiceData: function () {
            const self = this;
            return TempleAPI.get(`/sales/invoices/${this.invoiceId}`)
                .done((response) => {
                    if (response && response.success) {
                        self.invoiceData = response.data;
                        console.log('Invoice data loaded:', self.invoiceData);
                    }
                })
                .fail((error) => {
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select class="form-select" id="customer_id" required>
                                            ...
                                            ${customers.map(c => `
                                                <option value="${c.id}" ${c.id == (invoice.devotee_id || invoice.customer_id) ? 'selected' : ''}>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-end">
                                                    <input type="number" class="form-control form-control-sm text-end" 
                                                           id="shipping_charges" value="${invoice.shipping_charges || 0}" min="0" step="0.01" style="width: 100px; display: inline-block;">
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td class="text-end">
                                                    <input type="number" class="form-control form-control-sm text-end" 
                                                           id="other_charges" value="${invoice.other_charges || 0}" min="0" step="0.01" style="width: 100px; display: inline-block;">
                                                </td>
                                            </tr>
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <textarea class="form-control" id="terms_conditions" rows="3">${invoice.terms_conditions || ''}</textarea>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label">Notes</label>
                                        <textarea class="form-control" id="notes" rows="3">${invoice.notes || ''}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="modal-body">
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input type="number" class="form-control" id="addonTaxRate" value="0" step="0.01" min="0">
                                    </div>
                                    <div class="col-md-4">
        ...
        loadExistingItems: function () {
            ...
            // Convert existing items to the new format
            invoice.items.forEach((item) => {
                ...
                // Determine item type and get description
                if (item.item_type === 'package' || item.type === 'package' || item.package_id || item.sales_package_id) {
                    newItem.is_addon = false;
                    newItem.type = 'package';
                    newItem.item_type = 'package';
                    newItem.package_id = item.package_id || item.sales_package_id || item.item_id;
                    newItem.item_id = item.package_id || item.sales_package_id || item.item_id;
                    newItem.type_display = 'Package';
                    newItem.description = item.package?.package_name || item.package?.name || item.description || 'Package';
                } else if (item.item_type === 'sales_item' || item.type === 'sales_item' || item.sale_item_id || item.sales_item_id) {
                    newItem.is_addon = true;
                    newItem.type = 'sales_item';
                    newItem.item_type = 'sales_item';
                    newItem.item_id = item.sale_item_id || item.sales_item_id || item.item_id;
                    newItem.type_display = 'Sales Item (Addon)';
                    newItem.description = item.sales_item?.name_primary || item.salesItem?.name_primary || item.sale_item?.name_primary || item.description || 'Sales Item';
                } else if (item.item_type === 'product' || item.type === 'product' || item.product_id) {
                    newItem.is_addon = true;
                    newItem.type = 'product';
                    newItem.item_type = 'product';
                    newItem.item_id = item.product_id || item.item_id;
                    newItem.type_display = 'Product (Addon)';
                    newItem.description = item.product?.name || item.description || 'Product';
                    newItem.uom_id = item.uom_id;
                    newItem.uom_name = item.uom?.name || '';
                } else {
                    // Fallback based on is_addon flag
                    if (item.is_addon) {
                        newItem.type = item.item_type || 'sales_item';
                        newItem.item_type = item.item_type || 'sales_item';
                        newItem.item_id = item.product_id || item.sale_item_id || item.sales_item_id || item.item_id;
                        ...
                        newItem.type = 'package';
                        newItem.item_type = 'package';
                        newItem.package_id = item.sales_package_id || item.package_id || item.item_id;
                        newItem.item_id = newItem.package_id;
                        newItem.type_display = 'Package';
                    }
                }
        ...
        bindEvents: function () {
            ...
            $('#saveAddonBtn').on('click', () => {
                ...
                const taxRate = parseFloat($('#addonTaxRate').val()) || 0;
                const discount = parseFloat($('#addonDiscount').val()) || 0;
                
                // CALCULATE tax amount: (price * qty - discount) * (taxRate / 100)
        ...
        saveInvoice: function () {
            ...
            TempleCore.showLoading(true);
            TempleAPI.put(`/sales/invoices/${this.invoiceId}`, data)
                ...
                .fail(function (xhr) {
                    TempleCore.showToast(xhr.responseJSON?.message || 'Failed to update invoice', 'error');
                })
                .always(function () {
                    TempleCore.showLoading(false);
                });
