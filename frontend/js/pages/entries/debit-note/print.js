// js/pages/entries/debit-note/print.js
(function($, window) {
    'use strict';
    
    window.EntriesDebitNotePrintPage = {
        entryId: null,
        entryData: null,
        templeSettings: null,
        ...
        init: function(params) {
            ...
            if (!this.entryId) {
                TempleCore.showToast('Invalid debit note ID', 'error');
                ...
                TempleAPI.get(`/accounts/entries/${this.entryId}`)
                    .done(function(response) {
                        if (response.success) {
                            ...
                            // Validate it's a debit note entry
                            if (self.entryData.entrytype_id !== 6) {
                                reject(new Error('This is not a debit note entry'));
                                return;
                            }
                            resolve();
                        } else {
                            reject(new Error('Failed to load debit note data'));
                        }
                    })
                    .fail(function() {
                        reject(new Error('Error loading debit note'));
                    });
            });
        },
        ...
        generatePrintHTML: function() {
            ...
            const html = `
                ...
                <html>
                <head>
                    <title>Debit Note - ${entry.entry_code}</title>
                    <style>
                        @media print {
                ...
                <body>
                    ...
                    <table width="750" border="0" align="center" cellpadding="5" style="margin-top: 20px;">
                        <tr style="font-size:14px;">
                            <td width="150"><b>Debit Note No:</b></td>
                        ...
                        ${(entry.reference_no || entry.paid_to) ? `
                            <tr style="font-size:14px;">
                                ${entry.reference_no ? `
                                    <td><b>Reference Purchase Bill:</b></td>
                                    <td>${entry.reference_no}</td>
                                ` : '<td></td><td></td>'}
                                ${entry.paid_to ? `
                                    <td><b>Vendor/Supplier:</b></td>
                                    <td>${entry.paid_to}</td>
                                ` : '<td></td><td></td>'}
                            </tr>
                        ` : ''}
                    ...
                    ${entry.narration ? `
                        <table width="750" align="center" style="margin-top:20px;">
                            <tr>
                                <td style="font-size: 14px;"><b>Reason for Debit Note:</b></td>
                            </tr>
                            <tr>
