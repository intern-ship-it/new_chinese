// js/pages/entries/journal/print.js
(function($, window) {
    'use strict';
    
    window.EntriesJournalPrintPage = {
        entryId: null,
        entryData: null,
        templeSettings: null,
        ...
        init: function(params) {
            ...
            if (!this.entryId) {
                TempleCore.showToast('Invalid journal entry ID', 'error');
                ...
                TempleAPI.get(`/accounts/entries/${this.entryId}`)
                    .done(function(response) {
                        if (response.success) {
                            ...
                            // Validate it's a journal entry
                            if (self.entryData.entrytype_id !== 4) {
                                reject(new Error('This is not a journal entry'));
                                return;
                            }
                            resolve();
                        } else {
                            reject(new Error('Failed to load journal entry data'));
                        }
                    })
                    .fail(function() {
                        reject(new Error('Error loading journal entry'));
                    });
            });
        },
