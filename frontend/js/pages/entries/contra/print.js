// js/pages/entries/contra/print.js
(function($, window) {
    'use strict';
    
    window.EntriesContraPrintPage = {
        entryId: null,
        entryData: null,
        templeSettings: null,
        ...
        init: function(params) {
            ...
            if (!this.entryId) {
                TempleCore.showToast('Invalid contra entry ID', 'error');
                ...
                TempleAPI.get(`/accounts/entries/${this.entryId}`)
                    .done(function(response) {
                        if (response.success) {
                            ...
                            // Validate it's a contra entry
                            if (self.entryData.entrytype_id !== 3) {
                                reject(new Error('This is not a contra entry'));
                                return;
                            }
                            resolve();
                        } else {
                            reject(new Error('Failed to load contra entry data'));
                        }
                    })
                    .fail(function() {
                        reject(new Error('Error loading contra entry'));
                    });
            });
        },
        ...
        generatePrintHTML: function() {
            ...
            // Calculate totals and identify transfer accounts
            let totalDebit = 0;
            let totalCredit = 0;
            ...
            if (entry.entry_items && entry.entry_items.length > 0) {
                entry.entry_items.forEach(function(item) {
                    ...
                    // Identify transfer accounts
                    if (isDebit && !toAccount) {
                        toAccount = item.ledger;
                        transferAmount = amount;
                    } else if (!isDebit && !fromAccount) {
                        fromAccount = item.ledger;
                    }
            ...
            const amountInWords = this.numberToWords(transferAmount || totalDebit);
            ...
            const html = `
                ...
                <html>
                <head>
                    <title>Contra Voucher - ${entry.entry_code}</title>
                    <style>
                        @media print {
    ...
    : `<span style="color:red;font-weight:bold;">
         Unbalanced (Diff: ${currencySymbol} ${isNaN(difference) ? '0.00' : difference.toFixed(2)})
       </span>`
}
                    ...
                    ${fromAccount && toAccount ? `
                             ...
                             <table width="750" border="0" align="center" cellpadding="5" style="margin-top: 20px;">
                                <tr style="font-size:14px;">
                                    <td width="200"><b>Transfer From Account:</b></td>
                                    <td>${fromAccount.name} ${fromAccount.left_code && fromAccount.right_code ? 
                                        `(${fromAccount.left_code}/${fromAccount.right_code})` : ''}</td>
                                </tr>
                                <tr style="font-size:14px;">
                                    <td><b>Transfer To Account:</b></td>
                                    <td>${toAccount.name} ${toAccount.left_code && toAccount.right_code ? 
                                        `(${toAccount.left_code}/${toAccount.right_code})` : ''}</td>
                                </tr>
                                <tr style="font-size:14px;">
                                    <td><b>Transfer Amount:</b></td>
                                    <td>
                                        ${currencySymbol} ${this.formatCurrency(transferAmount)}
                                    </td>
                                </tr>
                            </table>
                    ...
                    <table width="750" align="center" style="margin-top:30px; border-collapse:collapse;">
                        <thead>
                            <tr style="font-size: 14px;">   
                                ...
                                <td width="150" style="border-top:2px solid black; border-bottom:2px solid black; padding:8px;" align="center"><b>Debit (${currencySymbol})</b></td>
                                <td width="150" style="border-top:2px solid black; border-bottom:2px solid black; padding:8px;" align="center"><b>Credit (${currencySymbol})</b></td>
                            </tr>
                        </thead>
                        <tbody>
                    ...
                    ${entry.payment || entry.cheque_no || entry.transaction_no ? `
                        <table width="750" align="center" style="margin-top:20px;">
                            <tr>
                                <td style="font-size: 14px;"><b>Payment Details:</b></td>
                            </tr>
                            <tr>
                                <td style="border:1px solid #ccc; padding:10px; font-size: 14px;">
                                    ${entry.payment ? `<b>Mode:</b> ${entry.payment}<br>` : ''}
                                    ${entry.cheque_no ? `<b>Cheque No:</b> ${entry.cheque_no}<br>` : ''}
                                    ${entry.cheque_date ? `<b>Cheque Date:</b> ${this.formatDate(entry.cheque_date)}<br>` : ''}
                                    ${entry.bank_name ? `<b>Bank:</b> ${entry.bank_name}<br>` : ''}
                                    ${entry.transaction_no ? `<b>Transaction No:</b> ${entry.transaction_no}<br>` : ''}
                                    ${entry.transaction_date ? `<b>Transaction Date:</b> ${this.formatDate(entry.transaction_date)}` : ''}
                                </td>
                            </tr>
                        </table>
                    ` : ''}
