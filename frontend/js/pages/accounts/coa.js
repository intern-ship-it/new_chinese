...
(function ($, window) {
    ...
    window.AccountsCoaPage = {
                        ...
                        <div class="row align-items-center">
                            <div class="col-md-6">
                             ...
                             <div class="col-md-6 text-md-end">
								...
								<button class="btn btn-secondary ms-2" id="manageFundsBtn">
									<i class="bi bi-wallet2"></i> Manage Funds
								</button>
            ...
            return `
				...
				<div class="modal fade" id="fundModal" tabindex="-1">
					<div class="modal-dialog modal-lg">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title">Fund Management</h5>
								<button type="button" class="btn-close" data-bs-dismiss="modal"></button>
							</div>
							<div class="modal-body">
								<!-- Add Fund Form -->
								<div class="card mb-3">
									<div class="card-header bg-light">
										<h6 class="mb-0" id="fundFormTitle">Add New Fund</h6>
									</div>
									<div class="card-body">
										<form id="fundForm">
											<div class="row">
												<div class="col-md-12">
													<div class="mb-3">
														...
														<input type="text" class="form-control" id="fundName" required maxlength="100">
													</div>
												</div>
											</div>
											<div class="mb-3">
												<label class="form-label">Description</label>
												<textarea class="form-control" id="fundDescription" rows="2" maxlength="500"></textarea>
											</div>
											<div class="text-end">
												<button type="button" class="btn btn-secondary" id="cancelFundEditBtn" style="display:none;">
													Cancel Edit
												</button>
												<button type="button" class="btn btn-primary" id="saveFundBtn">
													<i class="bi bi-check-circle"></i> Save Fund
												</button>
											</div>
										</form>
									</div>
        ...
        bindEvents: function () {
            ...
            $('#manageFundsBtn').on('click', function () {
                self.openFundModal();
            });
            ...
            $('#cancelFundEditBtn').on('click', function () {
                self.cancelFundEdit();
            });
        ...
        renderQuickActions: function (node) {
            ...
            if (this.permissions.can_create_group) {
                actionsHtml += `
                    <button class="btn btn-outline-primary" onclick="AccountsCoaPage.addSubGroup('${node.id}')">
            ...
            if (!data.fixed) {
                if (this.permissions.can_edit_group) {
                    actionsHtml += `
                        <button class="btn btn-outline-secondary" onclick="AccountsCoaPage.editGroup('${node.id}')">
                ...
                if (this.permissions.can_delete_group && data.children_count === 0 && data.ledgers_count === 0) {
                    actionsHtml += `
                        <button class="btn btn-outline-danger" onclick="AccountsCoaPage.deleteGroup('${node.id}')">
        ...
        showGroupModal: function (editId = null, parentId = null) {
            ...
            if (editId) {
                ...
                TempleAPI.get('/accounts/chart-of-accounts/groups/' + groupId)
                    .done(function (response) {
                        if (response.success) {
                            ...
                            // Load parent groups and then set the selected value
                            self.loadParentGroups(editId).done(function () {
                ...
                // Load parent groups first
                this.loadParentGroups(null).done(function () {
                    if (parentId) {
                        const actualParentId = parentId.replace('g_', '');
                        $('#parentGroup').val(actualParentId);
                        self.updateCodeRangeHelp(actualParentId);
                    }
                });
        ...
        // Load parent groups for dropdown - now returns a promise
        loadParentGroups: function (excludeId) {
            const self = this;

            return TempleAPI.get('/accounts/chart-of-accounts/hierarchical-groups', {
            ...
            TempleAPI.get('/accounts/funds')
                .done(function (response) {
                    if (response.success) {
                        ...
                        self.renderFundTable();
                    }
                })
                .fail(function () {
                    self.renderFundTableError('Failed to load funds');
                });
        },
        ...
        renderFundTable: function () {
                ...
                $('#fundTableBody').html(`
					<tr>
						<td colspan="4" class="text-center text-muted py-3">
							No funds found. Create your first fund above.
						</td>
					</tr>
				`);
                return;
            ...
            this.funds.forEach(function (fund) {
                html += `
					<tr>
						...
						<td>
							<button class="btn btn-sm btn-outline-primary" onclick="AccountsCoaPage.editFund(${fund.id})" title="Edit">
								<i class="bi bi-pencil"></i>
							</button>
							<button class="btn btn-sm btn-outline-danger" onclick="AccountsCoaPage.deleteFund(${fund.id})" title="Delete">
								<i class="bi bi-trash"></i>
							</button>
						</td>
					</tr>
            ...
            $('#fundTableBody').html(html);
        },

        renderFundTableError: function (message) {
            $('#fundTableBody').html(`
				<tr>
        ...
        saveFund: function () {
            ...
            const data = {
                fund_name: $('#fundName').val(),
                fund_description: $('#fundDescription').val(),
                status: $('#fundStatus').val()
            };

            TempleCore.showLoading(true);

            const request = this.editingFundId
                ? TempleAPI.put('/accounts/funds/' + this.editingFundId, data)
                : TempleAPI.post('/accounts/funds', data);

            request
                .done(function (response) {
                    if (response.success) {
                        TempleCore.showToast(
                            self.editingFundId ? 'Fund updated successfully' : 'Fund created successfully',
                            'success'
                        );
                        self.resetFundForm();
                        self.loadFunds();
        ...
        editFund: function (id) {
            ...
            $('#fundFormTitle').text('Edit Fund');
            $('#fundName').val(fund.name);
            $('#fundDescription').val(fund.description);
            $('#cancelFundEditBtn').show();
            $('#saveFundBtn').html('<i class="bi bi-check-circle"></i> Update Fund');

            // Scroll to form
            document.getElementById('fundForm').scrollIntoView({ behavior: 'smooth' });
        },

        cancelFundEdit: function () {
            this.resetFundForm();
        },

        resetFundForm: function () {
            this.editingFundId = null;
            $('#fundFormTitle').text('Add New Fund');
            $('#fundForm')[0].reset();
            $('#fundForm').removeClass('was-validated');
            $('#cancelFundEditBtn').hide();
            $('#saveFundBtn').html('<i class="bi bi-check-circle"></i> Save Fund');
        },
        ...
        deleteFund: function (id) {
            ...
            TempleCore.showConfirm(
                'Delete Fund',
                `Are you sure you want to delete the fund "<strong>${fund.fund_name}</strong>"?<br>
				<small class="text-muted">Note: Funds that are used in accounting entries cannot be deleted.</small>`,
                function () {
                    TempleCore.showLoading(true);

                    TempleAPI.delete('/accounts/funds/' + id)
                        .done(function (response) {
                            if (response.success) {
                                TempleCore.showToast('Fund deleted successfully', 'success');
                                AccountsCoaPage.loadFunds();
                            }
                        })
                        .fail(function (xhr) {
                            let message = 'Failed to delete fund';
                            if (xhr.responseJSON && xhr.responseJSON.message) {
                                message = xhr.responseJSON.message;
                            }
                            TempleCore.showToast(message, 'danger');
